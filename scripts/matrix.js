...
function randomize() {
    for (let i = 0; i < parseInt(numberOfRows.value); ++i) {
        for (let j = 0; j < parseInt(numberOfColumns.value); ++j) {
            document.querySelector('.A' + String(i) + String(j)).value = (75 * Math.random() - 75 * Math.random()).toFixed(3);
        }
    }
    if (document.querySelector('.B' + '0' + '0') !== null) {
        for (let i = 0; i < parseInt(numberOfRows.value); ++i) {
            for (let j = 0; j < parseInt(numberOfColumns.value); ++j) {
                document.querySelector('.B' + String(i) + String(j)).value = (75 * Math.random() - 75 * Math.random()).toFixed(3);
            }
        }
    }
}
...
function multiplicateOnNumber() {
    ...
    for (let i = 0; i < parseInt(numberOfRows.value); ++i) {
        for (let j = 0; j < parseInt(numberOfColumns.value); ++j) {
            arr.push((mlt * parseFloat(document.querySelector('.A' + String(i) + String(j)).value)).toFixed(3));
        }
    }
    return arr;
}
...
function add() {
    ...
    for (let i = 0; i < parseInt(numberOfRows.value); ++i) {
        for (let j = 0; j < parseInt(numberOfColumns.value); ++j) {
            arr.push((parseFloat(document.querySelector('.A' + String(i) + String(j)).value) + parseFloat(document.querySelector('.B' + String(i) + String(j)).value)).toFixed(3));
        }
    }
    return arr;
}
...
function multiplicate() {
    ...
    while (row < parseInt(numberOfRows.value)) {
        sum += parseFloat(document.querySelector('.A' + String(row) + String(i)).value) * parseFloat(document.querySelector('.B' + String(i) + String(j)).value);
        ++i;
        if (i === parseInt(numberOfRows.value)) {
            ++j,
            i = 0,
            resultMatrix.push(sum.toFixed(3)),
            sum = 0;
        }
...
function setResult(fn) {
    ...
    for (let i = 0; i < parseInt(numberOfColumns.value) * parseInt(numberOfRows.value); ++i) {
        ...
        (i + 1) % parseInt(numberOfColumns.value) === 0 ? outputField.appendChild(document.createElement("br")) : {};
    }
}
...
function calculate() {
    ...
    case "transpone":
        ...
        for (let i = 0; i < parseInt(numberOfColumns.value) * parseInt(numberOfRows.value); ++i) {
            ...
            res.value = rarr[i];
            outputField.appendChild(res);
            (i + 1) % parseInt(numberOfRows.value) === 0 ? outputField.appendChild(document.createElement("br")) : {};
        }
        break;
    case "add":
        setResult(add());
